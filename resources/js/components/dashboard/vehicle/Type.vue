			...
			<v-row justify="center">
				<v-col sm="12" md="12" lg="11" >
                    <v-toolbar flat color="white">
                        <v-toolbar-title>VehicleType List</v-toolbar-title>
                        <v-divider class="mx-2" inset vertical></v-divider>
                        <v-spacer></v-spacer>
                        <v-dialog v-model="dialog" persistent max-width="600px">
                                <template v-slot:activator="{ on }">
                                <v-btn color="primary" dark class="mb-2" v-on="on" @click="edit=true">New Vehicle Type</v-btn>
                                </template>
                                <v-card>
                                <v-card-title>
                                ...
                                <v-container>
                                    ...
                                    <v-col cols="12" sm="12" md="12" lg="12">
                                        <v-text-field label="Enter Vehicle Type"  
                                        v-model="editedItem.type"
                                        :rules="[v => !!v.length || 'Vehicle Type is required']"
                                        required
                                        filled
                                        ></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="12" md="12" lg="12">
                                        <v-text-field label="Enter Vehicle Type code"  
                                        v-model="editedItem.code"
                                        :rules="[v => !!v.length || 'VehicleType code is required']"
                                        required
                                        filled
                                        ></v-text-field>
                                    </v-col>
...
export default {
  ...
  computed: {
    formTitle() 
    {
      return this.editedIndex === -1 ? "New VehicleType" : "Edit VehicleType";
    },
        ...
        async save() 
            {
                ...
                try
                {
                   this.loading=true 
                    ...
                    this.vehicleImage.forEach(file => {
					formData.append("vehicleImage", file); // array in formData
                    });
                   ...
                   let { data } = await axios({
                   method: "post",
                   url: "/app/updatevehicletype/vehicletype",
                   data: formData
                  });
                 if(data.status)
                    {
                ...
                if (data.status) 
                    ...
                    {
						this.snacks("VehicleType Allready used in database", "red");
						this.loading = false;
					}
                } 
                catch (e) 
    ...
    async remove() 
        {
            try 
            {
    				let { data } = await axios({
					method: "delete",
					url: "/app/vehicletype/" + this.dataList[this.dataIndex].id
				    });
                    if (data.status) 
                    {
                        this.snacks('Successfully Done','green')
  ...
  watch:
    {
        files()
        {
                ...
                this.vehicleImage.push(files[0])
