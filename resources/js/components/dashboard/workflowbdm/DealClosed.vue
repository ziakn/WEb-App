		...
		<v-container fluid>
            ...
            <v-row>
                 ...
                 <v-dialog v-model="dialog2" persistent max-width="550px">
                   <v-card>
                        <v-card-title>
                        <span class="headline">Cancel Deal Remarks.....</span>
                        </v-card-title>
                        <v-card-text>
                        <v-container>
                            <v-row>
				...
				>
                               <v-text-field
                                    v-model="dealCancel.remark"
                                    label="Give A reason"
                                    :rules="[v => !!v.length || 'Reason is required']"
									required
                                    filled
                               ></v-text-field>
                                 </v-form>
                        ...
                        <v-btn color="primary" text @click="saveCancelDeal">Save</v-btn>
                        </v-card-actions>
                    </v-card>
                 </v-dialog>
                ...
                <v-col>
                    <v-toolbar flat color="white">
						<v-toolbar-title>Deal Closed (BDM)</v-toolbar-title>
                            ...
                            <v-data-table dense :headers="headers" :items-per-page="filters.show" :search="search" :items="dataList"  >
                                   ...
                                   <template v-slot:item.bde_id="{ item }">
                                    ...
                                    <template v-slot:item.action="{ item }">
                                        ...
                                        <v-tooltip bottom>
                                            <template v-slot:activator="{ on }">
                                              <v-icon small @click="dealCancelItem(item)" v-on="on">clear</v-icon>
                                            </template>
...
export default {
    ...
    methods: {
        ...
        async changeStatus(i)
          ...
          async saveCancelDeal()
            {

                if (this.$refs.form.validate()) {
                this.dealCancel.id=this.editedItem.id
                this.dealCancel.email=this.editedItem.email
                 try { 
                  let { data } = await axios({
                            method: "post",
                            url: "/app/canceldeal/bdm",
                            data: this.dealCancel
                        });
                        if(data.status)
                        {  this.snacks('Successfully Done','green')
                            this.$router.go()
