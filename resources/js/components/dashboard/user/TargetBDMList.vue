      ...
      <v-overlay :value="showFullLoading" :absolute="absolute">
        <v-progress-circular indeterminate size="64"></v-progress-circular>
         ...
         <v-row justify="center">
              ...
              <v-row>
                   <v-col xs="12" sm="12" md="6" lg="6">
                         <v-menu
                                    ...
                                    <v-date-picker  type="month" @change="getTargetListBDM" v-model="filters.month" @input="menu1= false"></v-date-picker>
                          </v-menu>
                  </v-col>
          ...
          <v-data-table :headers="headers" :items="dataTarget">
  ...
  methods: 
      ...
      async getTargetListBDM()
      {
            try {
              let { data } = await axios({
                method: "put",
                url: "/app/targetlist/bdm/" + this.$route.params.id,
                params: this.filters
              });
    ...
    async save() 
    {
      if (this.editedIndex > -1) 
      {
          try {
            let { data } = await axios({
              method: "put",
              url: "/app/target/updatebdm/" + this.dataTarget[this.editedIndex].id,
    ...
    async remove() 
    {
      ...
      try {
        let { data } = await axios({
          method: "delete",
          url: "/app/target/deletebdm/" + this.dataTarget[this.dataIndex].id
        });
        if (data.status) {
