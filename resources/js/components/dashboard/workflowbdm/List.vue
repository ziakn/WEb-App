            ...
            <v-row> 
                ...
                <v-col>
                    <v-toolbar flat color="white">
						<v-toolbar-title>Deal List (BDM)</v-toolbar-title>
                     ...
                     <v-card flat>
                        <v-card-text>
                            <v-data-table dense :headers="headers" :items-per-page="filters.show" :search="search" :items="dataList"  >
                                   ...
                                   <template v-slot:item.busde_id="{ item }">
        ...
        async changeStatus(i)
            ...
            close() {
             ...
             async save() {
                    if (this.editedIndex > -1) 
                    {
                        ...
                        let { data } = await axios({
                            method: "put",
                            url: "/app/bdm/" + this.dataList[this.editedIndex].id,
         ...
         async remove() {
                    try {
                        let { data } = await axios({
                            method: "delete",
                            url: "/app/bdm/" + this.dataList[this.dataIndex].id
                        });
                        this.snacks('Successfully Done','green')
