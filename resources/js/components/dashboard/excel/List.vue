		...
		<v-container>
            ...
            <v-row>
                 <v-dialog v-model="dialog" persistent max-width="800px">
                    <v-card>
                        <v-card-title>
                        <span class="headline">New Appoinment</span>
                        </v-card-title>
                        <v-card-text>
                             <v-form
                                ref="form"
                 ...
                 <v-dialog v-model="dialog1" persistent max-width="550px">
                    <v-card>
                        <v-card-title>
                        <span class="headline">Assign To Business Development Executive</span>
                        </v-card-title>
                        <v-card-text>
                        <v-container>
                            <v-row>
                            ...
                            <v-col cols="12" sm="12" md="12">
                                 <v-select
                                    v-model="editedItem.bde_id"
                                    :items="dataBDE"
                                    item-text="name"
                                    item-value="id"
                                    label="Select Business Development Executive"
                                    filled
                                ></v-select>
                            </v-col>
                            </v-row>
                        ...
                        <v-btn color="primary" text @click="saveBDE">Save</v-btn>
                        </v-card-actions>
                    </v-card>
                 </v-dialog>
                ...
                <v-col>
                    <v-toolbar flat color="white">
						<v-toolbar-title>Excel Sheet</v-toolbar-title>
                        ...
                        <v-card-text>
                            <v-data-table dense :headers="headers" :items-per-page="filters.show" :search="search" :items="dataList"  >
                                    ...
                                    <template v-slot:item.action="{ item }">
                                        <v-tooltip bottom>
                                            <template v-slot:activator="{ on }">
                                              <v-icon small @click="assignBDE(item)" v-on="on">assignment_ind</v-icon>
                                            </template>
                                            <span>Assign to BDE </span>
                                        </v-tooltip>
                                        <v-tooltip bottom>
                                            <template v-slot:activator="{ on }">
            ...
            async initialize()
             {
				...
				let { data } = await axios({
					method: "get",
					url: "/app/getproductoption"  
                });
                //   this.getChildproductoption = data;
                    }
                     catch (e) {
                        this.loading=false;
                    }
