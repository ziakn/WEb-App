			...
			<v-row justify="center">
				<v-col sm="12" md="12" lg="11" >
                    <v-toolbar flat color="white">
                        <v-toolbar-title>Customer Branch Zone List</v-toolbar-title>
                        <v-divider class="mx-2" inset vertical></v-divider>
                        <v-spacer></v-spacer>
                        <v-dialog v-model="dialog" persistent max-width="800px">
                                <template v-slot:activator="{ on }">
                                <v-btn color="primary" dark class="mb-2" v-on="on" @click="edit=true">Add Customer Zone Branch </v-btn>
                                </template>
                                <v-card>
                                <v-card-title>
                                ...
                                <v-container>
                                   ...
                                   <v-col cols="12" sm="6" md="6" lg="6">
                                        ...
                                        :rules="[v => !!v.length || 'Branch Name is required']"
                                        filled
                                        ></v-text-field>
                                    </v-col>
  ...
  methods: 
    {
        ...
        async initialize() 
            {
                ...
                this.getCustomerZoneBranchList();
         ...
         async getSumCustodyBottle() 
            {
                try 
                {
                    let { data } = await axios({
                    method: "get",
                    url: "/app/getonlysumcustodybottle/customerzonebranch/"+this.$route.params.id,
                    });
                    this.totalcustodyBottle = data;
                }
                catch (e) 
                {
            ...
            async getSumDespenserBottle() 
            {
                try 
                {
                    let { data } = await axios({
                    method: "get",
                    url: "/app/getonlysumcustodydespenser/customerzonebranch/"+this.$route.params.id,
                    });
                    this.totalcustodyDespenser = data;
                }
                catch (e) 
                {
         ...
         async getCustomerInformation() 
            {
                try 
                {
                    let { data } = await axios({
                    method: "get",
                    url: "/app/getcustomerinformation/customerzonebranch/"+this.$route.params.id,
                    });
                    this.dataCustomer = data;
                    this.custodyBottle =this.dataCustomer.custodybottle
                }
                catch (e) 
                {
        ...
        async getCustomerZoneBranchList() 
            {
                try 
                {
                    let { data } = await axios({
                    method: "get",
                    url: "/app/getcustomerzonebranchlist/customerzonebranch/"+this.$route.params.id,
                    });
                    this.dataList = data;
                }
                catch (e) 
        ...
        async save() 
            {
                ...
                try
                {
                     this.editedItem.customer_id= this.dataCustomer.id
                   this.loading=true 
                   let { data } = await axios({
                   method: "post",
                   url: "/app/updatecustomerzonebranchlist/customerzonebranch" ,
                 ...
                 if(data.status)
                    ...
                    {
                        this.snacks("Please Fill all the field", "red");
                        this.loading = false;
                    }
                } 
                catch (e) 
                ...
                try 
                {
                   ...
                   this.editedItem.customer_id= this.dataCustomer.id
                    ...
                    let { data } = await axios({
                    method: "post",
                    url: "/app/savecustomerzonebranch/customerzonebranch",
                    data: this.editedItem
                 });
                if (data.status) 
                    {
    ...
    async remove() 
        {
            try 
            {
    				let { data } = await axios({
					method: "delete",
					url: "/app/deletecustomerzonebranch/customerzonebranch/" + this.dataList[this.dataIndex].id
				    });
                    if (data.status) 
                    {
                        this.snacks('Successfully Done','green')
